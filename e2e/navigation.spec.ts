...
test.describe('Navigation', () => {
  ...
  test('should display logo and title in navbar', async ({ page }) => {
    await page.goto('/');

    // Check for logo
    const logo = page.locator('img[alt="Pulse Logo"]');
    await expect(logo).toBeVisible();

    // Check for title
    await expect(page.locator('h1:has-text("Pulse Survey Actions")')).toBeVisible();
